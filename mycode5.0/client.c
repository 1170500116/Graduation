...
/*****************/
static int get(char* key,size_t* value_len,char ** value,int i);
static int put(char* key,char * value,int i);
static int del(char* key,int i);
#define SEND_REGION_MAX_LEN 70
...
static int recv_server_metadata(int i){
	...
	server_metadata_mr = rdma_buffer_register(res[i]->pd,
...
int get(char key[KEY_LEN],size_t *value_len,char ** value,int i){ 
	...
	time_t time_now = time(NULL); /*获取time_t类型的当前时间*/
	char linshi_lock[LOCK_LEN+1];
	snprintf(linshi_lock,LOCK_LEN+1,"%016lx%08x%2s%08x\0",time_now,ioshu,IPbuffer+ip_offset,pid);
	snprintf(res[i]->send_region, SEND_REGION_MAX_LEN, "0%-16s%34s%08x\0",key,linshi_lock,period);
	on_connection(i);
	...
	*value = res[i]->dst+24;
    ...
    snprintf(res[i]->send_region, SEND_REGION_MAX_LEN, "3%-16s%34s\0",key,linshi_lock);
    on_connection(i);
    num = res[i]->recv_region[0]-'0';
    if(num==0){
...
int put(char* key,char * value,int i){
	...
	char linshi_lock[LOCK_LEN+1];
	snprintf(linshi_lock,LOCK_LEN+1,"%016lx%08x%2s%08x\0",time_now,ioshu,IPbuffer+ip_offset,pid);
	snprintf(res[i]->send_region, SEND_REGION_MAX_LEN, "1%-16s%08lx%34s%08x\0",key,record_len,linshi_lock,period);
    ...
    while(num!=1){
        ...
        usleep(rand()%100000);//sleep < 100000us ,0.1s
        time_now = time(NULL);
        snprintf(linshi_lock,LOCK_LEN+1,"%16lx%08x%2s%08x\0",time_now,ioshu,IPbuffer+ip_offset,pid);
	    snprintf(res[i]->send_region, SEND_REGION_MAX_LEN, "1%-16s%08lx%34s%08x\0",key,record_len,linshi_lock,period);
...
int del(char key[KEY_LEN],int i){ 
	...
	snprintf(res[i]->send_region, SEND_REGION_MAX_LEN, "2%-16s%34s\0",key,linshi_lock);
	on_connection(i);
	int num = res[i]->recv_region[0]-'0';
    while(num==2){      
        usleep(rand()%100000);//sleep < 100000us ,0.1s
...
//typedef clock_t long 4 bytes
	...
	for(  i = 0;i<80000;i++){
		...
		put( _key,_value,myserverid);	
        ioshu++;
		get(_key,&ans_value_len,&ans_value,myserverid);
        ioshu++;
		finish = clock();  
		...
		if(rand()%2){
			if(del(_key,myserverid)==0){ans_error++;};
            ioshu++;
			if(get(_key,&ans_value_len,&ans_value,myserverid)){ans_error++;};
            ioshu++;
		}
        itimes++;	
	}
	...
	printf( "io:%d , iops :%f t/s~~~~~~~~~~~~~~~~~~~\n", ioshu,ioshu/(duration/CLOCKS_PER_SEC)); 
