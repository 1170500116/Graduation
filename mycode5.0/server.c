...
void parse_command(char * str){ 	
    ...
    HASH_VALUE hash_val = check_hashval(key);
   if(hash_val==(ZONG_HASH_MAP_SIZE/server_num)){
       snprintf(send_region, SEND_REGION_MAX_LEN,"error\0");
       return;
   }
    //printf("%d :%s\n",num,key);
    // num = 0: get
    ...
    // num = 3: del_lock
    if(num==0){
        // snprintf(res[i]->send_region, SEND_REGION_MAX_LEN, "0%-16s%34%08x\0",key,linshi_lock,period);
     ...
     //   printf("linshi_lock:%s.len:%d\n",linshi_lock,strlen(linshi_lock));
        ...
        snprintf(send_region, SEND_REGION_MAX_LEN, "%1d%08x%08lx\0",ret,block_index*BLOCK_SIZE,record_len); 
    }else if(num==1){


        //  snprintf(send_region, SEND_REGION_MAX_LEN, "1%-16s%08x%34s%08x\0",key,record_len,linshi_lock,period);

        // char * start_time_str= malloc(16);
        // memcpy(start_time_str,str+25,16);
        char * linshi_lock= malloc(LOCK_LEN);    
        memcpy(linshi_lock,str+25,LOCK_LEN);
        char * record_len_str = malloc(8);
        memcpy(record_len_str,str+17,8);
        // char * period_str = malloc(8);
        // memcpy(period_str,str+51,8);
       
        char * leftover;
        VALUE_LEN_TYPE record_len = strtoul (record_len_str, &leftover, 16);
        //time_t start_time = strtoul (start_time_str, &leftover, 16);
        //time_t period = strtoul (period_str, &leftover, 16);	
        //printf("value_size:%d\n",value_size);
        BLOCK_INDEX_TYPE block_index = 0;
