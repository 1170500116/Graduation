...
int get(char key[KEY_LEN],size_t *value_len,char ** value,int i){ 
	...
	snprintf(res[i]->send_region, SEND_REGION_MAX_LEN, "0%-16s\0",key);
	on_connection(i);
...
int put(char* key,char * value,int i){
	...
	snprintf(res[i]->send_region, SEND_REGION_MAX_LEN, "1%-16s%08x\0",key,record_len);
	//snprintf(send_region, SEND_REGION_MAX_LEN, "1%-16s%08x%16x%08x%2s%08x%08x\0",key,record_len,(time_t signed long 8bytes)time,("43")jiqihaoma,(int)xianchengid,(int)shiyongshijan);
	on_connection(i);
	int num = res[i]->recv_region[0]-'0';
	if(num==0){
		return 0;
	...
	snprintf(res[i]->src,BUFFER_SIZE,"%08x%-16s%s\0\0\0\0\0\0",strlen(value),key,value); 
...
int main(int argc, char **argv) {	
	...
	// time_t timep = time(NULL); /*获取time_t类型的当前时间*/
	// printf("%ld",timep);
	char _key[KEY_LEN+1] ; 
