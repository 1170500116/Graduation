...
excuteFun(){
      ...
      expect -d -c"
                set timeout 300               
             spawn scp -3 -r ./mycode5.0 ${user}@$ip:/home/wanru
       ...
       expect -d -c"
            ...
            send \"cd ~/mycode5.0/\r\"
            send \"sed -i 's/#define HASH_MIN 750/#define HASH_MIN $a/g' nvm_engine.h\r\"
            send \"rm -f db1&&make clean&&make\r\"         
            send \"exit\r\"
            expect eof"
...
do
   ...
   #  opera='chmod 777 /usr/local/hadoop/zookeeper-3.4.12/conf/'
    ...
    # pssh -H ${user}@$ip -P  'cd ~/mycode5.0/ && ./server \r'
